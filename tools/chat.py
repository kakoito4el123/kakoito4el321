...
from tools.async_helpers import run_async
...
def create_chat(parent, on_logout=None):
    ...
    def send_msg(e=None):
        ...
        target = current_chat_with
        final = f"➥ {reply_data}\n{text}" if reply_data else text
        entry.delete(0, tk.END)
        close_reply()

        # 1. Сразу рисуем сообщение локально со статусом "отправляется" (один тик)
        bubble = tk.Label(txt_output, text=f"{final}  🕓", bg=theme["bubble_me"],
                           fg="black" if theme["name"] == "light" else "white",
        ...
        txt_output.tag_add("right", line_start, "insert")
        txt_output.insert(tk.END, "\n\n")
        txt_output.config(state=tk.DISABLED)
        txt_output.see(tk.END)

        # 2. В фоне реально сохраняем в Supabase
        def work():
            save_message(my_nick, target, final)
            return True

        def done(_result):
            if current_chat_with == target:
                bubble.config(text=f"{final}  ✓✓")  # доставлено на сервер
                update_chat_window()

        run_async(txt_output, work, done)
    ...
    def update_chat_window():
        ...
        loading_lbl = tk.Label(txt_output, text="⏳ Загрузка...", bg=theme["chat_bg"], fg=theme["muted_text"])
        txt_output.config(state=tk.NORMAL)
        txt_output.delete('1.0', tk.END)
        txt_output.window_create(tk.END, window=loading_lbl)
        txt_output.config(state=tk.DISABLED)

        target = current_chat_with

        def fetch():
            return get_chat_history(my_nick, target)

        def render(history):
            nonlocal last_msg_count
            if current_chat_with != target:
                return  # пользователь уже переключился на другой чат, не рисуем устаревшее
            if isinstance(history, Exception):
                history = []
            last_msg_count = len(history)
            txt_output.config(state=tk.NORMAL)
            txt_output.delete('1.0', tk.END)
            txt_output._img_refs = []
            for sender, content, time, is_image, msg_id in history:
                ...
                if is_image:
                    try:
                        ...
                        txt_output.insert(tk.END, " [Ошибка фото] ")
                else:
        ...
        run_async(txt_output, fetch, render)
