...
def show_auth_window(root, on_success, on_code=None):
    ...
    # Локальные диалоги не зависят от системного messagebox в отдельном потоке.
    def show_error(title, message):
        win = tk.Toplevel(auth_win)
        win.title(title)
        win.geometry("300x120")
        win.configure(bg="#1f2937")
        win.attributes("-topmost", True)
        win.grab_set()
        tk.Label(win, text=message, bg="#1f2937", fg="white", wraplength=260,
                font=("Segoe UI", 10), justify="center").pack(expand=True, padx=15, pady=15)
        tk.Button(win, text="OK", command=win.destroy, bg="#374151", fg="white",
                relief=tk.FLAT).pack(pady=(0, 15))
    ...
    def handle_login():
        ...
        if login_user(nick, pw):
            ...
            show_info("Успех", "Вы успешно вошли!")
            auth_win.destroy()
            on_success()
        else:
    ...
    def finalize_reg():
        ...
        if user_code == generated_code:
            if register_user(nick, phone, pw):
                ...
                show_info("Успех", "Аккаунт успешно создан!")
                auth_win.destroy()
                on_success()
            else:
    ...
    tk.Label(code_frame, text="Введите код подтверждения:", fg="#e74c3c", font=("Arial", 10, "bold")).pack()
